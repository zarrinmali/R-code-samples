...
Main steps: 1.
DOA internal matching: - Splits the raw DOA dataset into records with and without facility IDs.
...
2.
DOA–EPA cross-agency matching: - Fuzzy-joins DOA and EPA datasets by facility name, filters by same county, and groups matches by string similarity.
...
3.
Final dataset - Collapses the merged dataset so that each facility corresponds to one row, consolidating similar facility and owner names using string distance metrics.
